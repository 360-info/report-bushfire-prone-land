  ...
  window.addEventListener("load", () => {
    ...
    if (edshare === null) {
      ...
      edshareBtn.addEventListener("click", () => {
        ...
        // get potential url fragment to add to embed code
        const pageURL = new URL(window.location.href);
        let fragments = [];
        const stateChoice = pageURL.searchParams.get("state");
        if (stateChoice !== undefined) {
          fragments.push("month=" + stateChoice);
        }
        const fragmentString = fragments.join("&");

        // copy the embed code to the clipboard
        navigator.clipboard.writeText(
          `{{< meta 360embed.before >}}{{< meta 360embed.url >}}?${fragmentString}{{< meta 360embed.after >}}`);
