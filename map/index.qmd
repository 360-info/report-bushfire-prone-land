...
stateStats = state.filter(d => d.STE_CODE21 == selectedStateCode)
```
...
async function zoomToSuburb(suburb) {
  ...
  // if not found, zoom out and try again (doesn't work)
  if (targetSuburbFeature.length == 0) {
    map.jumpTo(
      map.cameraForBounds(initBounds[selectedState], {
        padding: {top: 100, bottom: 5, left: 5, right: 5},   
      }));

    targetSuburbFeature = await Promises.delay(4000,
      map.querySourceFeatures("suburbs-source", {
        sourceLayer: "suburbs",
        filter: ["==", "SAL_NAME21", suburb]
      }));
    console.log("Features found second time:", targetSuburbFeature)
  }
